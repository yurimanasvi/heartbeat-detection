...
st.markdown("AI-powered victim detection · College Project")
st.markdown("---")
...
with st.sidebar:
    ...
    if st.button("🔄 Load & train AI model", use_container_width=True):
        with st.spinner("Training on 600 samples…"):
...
st.markdown("---")
st.markdown('<p style="text-align:center;font-size:12px;color:#aaa;">Heartbeat Disaster Detection · College Project · Simulated data only</p>', unsafe_allow_html=True)
